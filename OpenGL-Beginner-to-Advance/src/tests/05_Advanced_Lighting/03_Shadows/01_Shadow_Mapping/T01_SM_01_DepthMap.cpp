#include "T01_SM_01_DepthMap.h"
#include "GLFW/glfw3.h"

namespace test {

	T01_SM_01_DepthMap::T01_SM_01_DepthMap()
		: m_f_fov(45.0f),
		...
		m_SHADOW_WIDTH(1024), m_SHADOW_HEIGHT(1024)
	{
		...
		m_ShaderMesh = std::make_shared<Shader>("src/tests/05_Advanced_Lighting/03_Shadows/01_Shadow_Mapping/S01_SM_01_DepthMap_A_BlinnPhong.Shader");
		...
		m_ShaderA_DepthMap = std::make_shared<Shader>("src/tests/05_Advanced_Lighting/03_Shadows/01_Shadow_Mapping/S01_SM_01_DepthMap_B_DepthMap.Shader");
		// 
		// FrameBuffer
		...
		m_ShaderScreen = std::make_shared<Shader>("src/tests/05_Advanced_Lighting/03_Shadows/01_Shadow_Mapping/S01_SM_01_DepthMap_C_Texture.Shader");
	...
	void T01_SM_01_DepthMap::OnUpdate(float deltaTime)
	{
	}

	void T01_SM_01_DepthMap::OnRender(GLFWwindow* window)
	{
		...
		glm::mat4 lightView = glm::lookAt(glm::vec3(-2.0f, 4.0f, -1.0f), glm::vec3(.0f, .0f, .0f), glm::vec3(.0f, 1.0f, .0f));
	...
	void T01_SM_01_DepthMap::OnImGuiRender()
	{
	...
	void T01_SM_01_DepthMap::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T01_SM_01_DepthMap::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
		if (!window)
		{
			// 
	...
	void T01_SM_01_DepthMap::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
	...
	void T01_SM_01_DepthMap::SetLighting(std::shared_ptr<Shader> shader)
	{
	...
	void T01_SM_01_DepthMap::DrawCube(std::shared_ptr<Shader> shader, glm::mat4 & model, glm::mat4 & view, glm::mat4 & proj, bool rotating, bool depthmap)
	{
		if (rotating)
		{
	...
	void T01_SM_01_DepthMap::FramebufferSetup(int width, int height)
	{
		if (!m_fbo.InitializeDepthMap(width, height))
