#include "T05_LightCaster_03_SpotL.h"
#include "GLFW/glfw3.h"

namespace test {

	T05_LightCaster_03_SpotL::T05_LightCaster_03_SpotL()
		: m_f_fov(45.0f),
		...
		m_b_outerCutOff_disable(false)
	{
		...
		m_Shader = std::make_shared<Shader>("src/tests/02_Lighting/05_LightCasters/S05_LightCaster_03_SpotL.Shader");
	...
	void T05_LightCaster_03_SpotL::OnUpdate(float deltaTime)
	{
	}

	void T05_LightCaster_03_SpotL::OnRender(GLFWwindow* window)
	{
		...
		{	// Object
			...
			m_Shader->SetUniform3fv("light.direction", m_camera->GetCamFront());
			m_Shader->SetUniform3fv("light.position", m_camera->GetCamPosition());
			...
			m_Shader->SetUniform1i("u_b_outerCutOff_OFF", m_b_outerCutOff_disable);
	...
	void T05_LightCaster_03_SpotL::OnImGuiRender()
	{
		...
		ImGui::Checkbox("DISACTIVE smoth outerCutOff", &m_b_outerCutOff_disable);
	}

	void T05_LightCaster_03_SpotL::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T05_LightCaster_03_SpotL::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
	}

	void T05_LightCaster_03_SpotL::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
