...
namespace test {
		...
		m_lightPos(glm::vec3(1.2f, 1.0f, 2.0f))
	{
		...
		m_camera = std::make_unique<Camera>(glm::vec3(-1.0f, 2.0f, 4.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.f, -20.f);
		...
		m_lightShader = std::make_unique<Shader>("src/tests/02_Lighting/01_Colors/S01_Color_01_Light.Shader");
