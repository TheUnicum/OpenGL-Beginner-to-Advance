#include "T02_Basic_Lighting_06_Scale.h"
#include "GLFW/glfw3.h"

namespace test {

	T02_Basic_Lighting_06_Scale::T02_Basic_Lighting_06_Scale()
		: m_f_fov(45.0f),
		...
		m_b_gouraud_shading_active(false),
		m_b_traspose_disable(false)
	{
		...
		m_VertexBuffer = std::make_unique<VertexBuffer>(pyramid_vertices_3v_3n, 24 * (3 + 3) * sizeof(float));
		...
		m_Shader = std::make_shared<Shader>("src/tests/02_Lighting/02_BasicLighting/S02_Basic_Lighting_06_Scale.Shader");
		m_Shader_Gouraud = std::make_shared<Shader>("src/tests/02_Lighting/02_BasicLighting/S02_Basic_Lighting_04_Gouraud.Shader");
	...
	void T02_Basic_Lighting_06_Scale::OnUpdate(float deltaTime)
	{
	}

	void T02_Basic_Lighting_06_Scale::OnRender(GLFWwindow* window)
	{
		...
		float inv_ratio_aspect = 960.0f / 540.0f;
		{
			...
			if (m_b_gouraud_shading_active)
				m_currentShader = m_Shader_Gouraud;
			else
			...
			mvp = proj * view * model;
			m_currentShader->SetUniformMat4f("u_mvp", mvp);
			m_currentShader->SetUniformMat4f("u_model", model);
			m_currentShader->SetUniformMat4f("u_view", view);

			if (!m_b_traspose_disable)
				m_currentShader->SetUniformMat3f("u_transInvers_model", glm::mat3(glm::transpose(glm::inverse(model))));
			else
				m_currentShader->SetUniformMat3f("u_transInvers_model", model);

			m_currentShader->SetUniform3f("objectColor", 1.0f, 0.5f, 0.31f);
			m_currentShader->SetUniform3f("lightColor", 1.0f, 1.0f, 1.0f);
			...
			m_currentShader->SetUniform3fv("lightPos", m_lightPos);
			m_currentShader->SetUniform3fv("viewPos", m_camera->GetCamPosition());

			// Active Phong lighting componetst
			m_currentShader->SetUniform1i("u_b_ambient", m_b_ambient);
			m_currentShader->SetUniform1i("u_b_diffuse", m_b_diffuse);
			m_currentShader->SetUniform1i("u_b_specular", m_b_specular);

			renderer.Draw(*m_VAO, 24, *m_currentShader);
			...
			renderer.Draw(*m_lightVAO, 24, *m_lightShader);
		}
	}

	void T02_Basic_Lighting_06_Scale::OnImGuiRender()
	{
		ImGui::Text("Color");
		...
		ImGui::Checkbox("Gouraud Shading active", &m_b_gouraud_shading_active);
		ImGui::Checkbox("Transpose matrix disable", &m_b_traspose_disable);
	}

	void T02_Basic_Lighting_06_Scale::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T02_Basic_Lighting_06_Scale::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
	}

	void T02_Basic_Lighting_06_Scale::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
