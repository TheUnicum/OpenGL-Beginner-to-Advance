#include "T06_Cubemaps_03_Model2.h"
#include "GLFW/glfw3.h"

namespace test {

	T06_Cubemaps_03_Model2::T06_Cubemaps_03_Model2()
		: m_f_fov(45.0f),
		m_b_firstMouse(true),
		m_mouse_lock(false),
		...
		m_framebufferWidth(WINDOW_WIDTH), m_framebufferHeight(WINDOW_HEIGHT)
	{
		...
		m_ShaderMesh = std::make_unique<Shader>("src/tests/04_Advanced_OpenGL/06_Cubemaps/S06_Cubemaps_03_Model2.Shader");
		m_ShaderBox = std::make_unique<Shader>("src/tests/04_Advanced_OpenGL/06_Cubemaps/S06_Cubemaps_02_Reflecion.Shader");
	...
	void T06_Cubemaps_03_Model2::OnUpdate(float deltaTime)
	{
	}

	void T06_Cubemaps_03_Model2::OnRender(GLFWwindow* window)
	{
	...
	void T06_Cubemaps_03_Model2::OnImGuiRender()
	{
	...
	void T06_Cubemaps_03_Model2::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T06_Cubemaps_03_Model2::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
		if (!window)
		{
			// 
	...
	void T06_Cubemaps_03_Model2::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
	...
	void T06_Cubemaps_03_Model2::DrawCube(glm::mat4 model, glm::mat4 proj, glm::mat4 view)
	{
		m_ShaderBox->Bind();
		m_cubemapTexture.Bind(1);
		m_ShaderBox->SetUniform1i("skybox", 1);
		...
		m_ShaderBox->SetUniformMat4f("u_mvp", mvp);
		m_ShaderBox->SetUniformMat4f("u_model", model);
		m_ShaderBox->SetUniformMat3f("u_transInvers_model", glm::mat3(glm::transpose(glm::inverse(model))));
		// View position
		m_ShaderBox->SetUniform3fv("u_viewPos", m_camera->GetCamPosition());

		// Reflect/Refraction
		m_ShaderBox->SetUniform1i("u_b_cube_boxtexture", m_b_cube_boxtexture);
		m_ShaderBox->SetUniform1i("u_b_refraction", m_b_refraction);
		m_ShaderBox->SetUniform1f("u_f_transparency", m_f_transparency);
		
		// Draw MESH
		m_mesh->Draw(m_ShaderBox);
	}

	void T06_Cubemaps_03_Model2::DrawModel(glm::mat4 model, glm::mat4 proj, glm::mat4 view)
	{
		m_ShaderMesh->Bind();
		m_cubemapTexture.Bind(1);
