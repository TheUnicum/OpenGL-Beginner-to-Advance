#include "T05_Framebuffers_05_Multi.h"
#include "GLFW/glfw3.h"

namespace test {

	T05_Framebuffers_05_Multi::T05_Framebuffers_05_Multi()
		: m_f_fov(45.0f),
		...
		m_framebufferWidth(WINDOW_WIDTH), m_framebufferHeight(WINDOW_HEIGHT)
	{
		...
		m_ShaderMesh = std::make_unique<Shader>("src/tests/04_Advanced_OpenGL/05_Framebuffers/S05_Framebuffers_05_Multi.Shader");
		...
		m_ShaderScreen = std::make_unique<Shader>("src/tests/04_Advanced_OpenGL/05_Framebuffers/S05_Framebuffers_05_Multi_Screeen.Shader");
	...
	void T05_Framebuffers_05_Multi::OnUpdate(float deltaTime)
	{
	}

	void T05_Framebuffers_05_Multi::OnRender(GLFWwindow* window)
	{
		...
		RenderFramebufferTexture(glm::vec3(0.5), glm::vec3(-0.5f, +0.5f, 0.0f), ((m_i_postproc_effect + 0) % 7));
		RenderFramebufferTexture(glm::vec3(0.5), glm::vec3(+0.5f, +0.5f, 0.0f), ((m_i_postproc_effect + 1) % 7));
		RenderFramebufferTexture(glm::vec3(0.5), glm::vec3(-0.5f, -0.5f, 0.0f), ((m_i_postproc_effect + 2) % 7));
		RenderFramebufferTexture(glm::vec3(0.5), glm::vec3(+0.5f, -0.5f, 0.0f), ((m_i_postproc_effect + 3) % 7));

	}

	void T05_Framebuffers_05_Multi::OnImGuiRender()
	{
	...
	void T05_Framebuffers_05_Multi::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T05_Framebuffers_05_Multi::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
		if (!window)
		{
			// 
	...
	void T05_Framebuffers_05_Multi::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
	...
	void T05_Framebuffers_05_Multi::FramebufferSetup(int width, int height)
	{
		if (!m_fbo.Initialize(width, height))
			std::cout << "ERROR::FRAMEBUFFERS:: Framebuffer is not complete!" << std::endl;
	}

	void T05_Framebuffers_05_Multi::RenderFramebufferTexture(glm::vec3 scale, glm::vec3 position, int index)
	{
		m_ShaderScreen->Bind();
