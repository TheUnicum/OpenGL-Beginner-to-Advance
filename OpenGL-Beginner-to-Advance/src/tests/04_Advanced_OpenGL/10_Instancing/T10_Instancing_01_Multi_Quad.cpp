#include "T10_Instancing_01_Multi_Quad.h"
...
namespace test {
	...
	T10_Instancing_01_Multi_Quad::T10_Instancing_01_Multi_Quad()
		: m_f_fov(45.0f),
		m_b_firstMouse(true),
		m_mouse_lock(false),
		...
		m_framebufferWidth(WINDOW_WIDTH), m_framebufferHeight(WINDOW_HEIGHT)
	{
		...
		m_Shader = std::make_unique<Shader>("src/tests/04_Advanced_OpenGL/10_Instancing/S10_Instancing_01_Multi_Quad.Shader");
	...
	void T10_Instancing_01_Multi_Quad::OnUpdate(float deltaTime)
	{
	}

	void T10_Instancing_01_Multi_Quad::OnRender(GLFWwindow* window)
	{
		...
		// Model
		{
			...
			// store offset into uniform !!
			// WARNING , max Uniform 4092!
			for (unsigned int i = 0; i < 100; i++)
			{
				...
				ss << i;
				index = ss.str();
				m_Shader->SetUniform2fv(("offsets[" + index + "]").c_str(), translations[i]);
			}
	...
	void T10_Instancing_01_Multi_Quad::OnImGuiRender()
	{
	...
	void T10_Instancing_01_Multi_Quad::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T10_Instancing_01_Multi_Quad::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
		if (!window)
		{
			// 
	...
	void T10_Instancing_01_Multi_Quad::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
