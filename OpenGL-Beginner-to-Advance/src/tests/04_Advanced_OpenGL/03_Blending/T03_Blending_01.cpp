...
namespace test {
	...
	T03_Blending_01::T03_Blending_01()
		...
		m_b_firstMouse(true),
		m_mouse_lock(false),
		m_i_manually_discard(false), m_i_optimize_clampToEdge_ON(false)
	{
		// Initialize camera
		...
		msp_mTextureGrass = std::make_shared<Texture>("res/textures/grass.png", TextureType::DIFFUSE);
		msp_mTextures3.push_back(msp_mTextureGrass);
		m_mesh_grass = std::make_unique<Mesh>(vertices_quad_3v_3n_2t, vec_quad_indices, msp_mTextures3);

		msp_mTextureGrass_Clamped = std::make_shared<Texture>("res/textures/grass.png", TextureType::DIFFUSE,
			GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
		msp_mTextures3C.push_back(msp_mTextureGrass_Clamped);
	...
	void T03_Blending_01::OnRender(GLFWwindow* window)
	{
		...
		{	// Grass
			...
			m_ShaderMesh->SetUniform1i("u_i_manually_discard", m_i_manually_discard);

			if (!m_i_optimize_clampToEdge_ON)
				m_mesh_grass->Draw(m_ShaderMesh);
			else
				m_mesh_grass_Clamped->Draw(m_ShaderMesh);
		}
