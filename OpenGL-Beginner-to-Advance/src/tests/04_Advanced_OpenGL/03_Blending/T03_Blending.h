...
namespace test {
	...
	private:
		...
		// Grass
		std::shared_ptr<Texture> msp_mTextureGrass, msp_mTextureGrass_Clamped;
		std::vector<std::shared_ptr<Texture>> msp_mTextures3, msp_mTextures3C;

		// MODEL
		std::unique_ptr<Model> m_model;

		// Blending
		bool m_i_manually_discard;
		bool m_i_optimize_clampToEdge_ON;
	};
