#include "T03_Mesh_02_ibo.h"
#include "GLFW/glfw3.h"

namespace test {

	T03_Mesh_02_ibo::T03_Mesh_02_ibo()
		: m_f_fov(45.0f),
		...
		m_b_draw_object(true), m_b_draw_mesh(true), m_b_draw_quad(true)
	{
		// Initialize camera
		m_camera = std::make_unique<Camera>(glm::vec3(-2.0f, 4.0f, 4.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.f, -20.f);
		...
		m_Shader = std::make_shared<Shader>("src/tests/03_Model_Loading/02_Mesh/S02_Mesh_01.Shader");
		m_lightShader = std::make_shared<Shader>("src/tests/03_Model_Loading/02_Mesh/S00_Color_01a_Light.Shader");
		...
		m_ShaderMesh = std::make_unique<Shader>("src/tests/03_Model_Loading/02_Mesh/S03_Mesh_02_ibo.Shader");
		...
		m_mesh_quad = std::make_unique<Mesh>(vertices_quad_3v_3n_2t, vec_quad_indices, msp_Textures);
	}
	...
	void T03_Mesh_02_ibo::OnUpdate(float deltaTime)
	{
	}

	void T03_Mesh_02_ibo::OnRender(GLFWwindow* window)
	{
		...
		if (m_b_draw_quad)
		{
			...
			// Draw MESH
			m_mesh_quad->Draw(m_ShaderMesh, m_b_draw_quad_glDrawArrays);
		}
	...
	void T03_Mesh_02_ibo::OnImGuiRender()
	{
		...
		ImGui::Text("Draw Object options:");
		ImGui::Checkbox("-Vertex Object", &m_b_draw_object);
		ImGui::Checkbox("-Mesh VertexBuffer only", &m_b_draw_mesh);
		ImGui::Checkbox("-Mesh Vertex/IndexBuffer", &m_b_draw_quad);
		ImGui::Text("Exercises A - motion options");
		ImGui::Checkbox("Qaud draw in glDrawArrays ", &m_b_draw_quad_glDrawArrays);

	}

	void T03_Mesh_02_ibo::OnProcessInput(GLFWwindow * window, float deltaTime)
	{
	...
	void T03_Mesh_02_ibo::framebuffer_size_callback(GLFWwindow * window, int width, int height)
	{
	}

	void T03_Mesh_02_ibo::mouse_callback(GLFWwindow * window, double xpos, double ypos)
	{
